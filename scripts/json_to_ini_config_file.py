...
def json_to_ini_config(config: Any, base_name: str, is_top_object: bool=True, out: List[Any]=[]) -> Any:
    if isinstance(config, dict):
        ...
        *pieces, list_name = base_name.split('.')
        parent_name = ".".join(pieces)

        out.extend([
            '',
            f'{parent_name}.num_{list_name} = {len(config)}'
        ])
...
if len(sys.argv) == 3:
    ...
    base_config_contents = Path.cwd() \
        .joinpath(base_config_file) \
        .resolve() \
        .read_text() \
        .splitlines()

    # This is needed because Python's ini file reader
    # (configparser.ConfigParser) does not view `include`
    # statements as valid.
    base_config.read_string('\n'.join(
        l for l in base_config_contents if not l.startswith('include ')
    ))
